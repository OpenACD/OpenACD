...
-module(call_queue_config_mnesia).
-author("Micah").
...
start() ->
	build_tables(),
	cpx_hooks:set_hook(mn_get_queue, get_queue, ?MODULE, get_queue, [], 100),
	cpx_hooks:set_hook(mn_get_queues, get_queues, ?MODULE, get_queues, [], 100),

	cpx_hooks:set_hook(mn_get_queue_group, get_queue_group, ?MODULE, get_queue_group, [], 100),
	cpx_hooks:set_hook(mn_get_queue_groups, get_queue_groups, ?MODULE, get_queue_groups, [], 100),

	cpx_hooks:set_hook(mn_get_skill, get_skill, ?MODULE, get_skill, [], 100),
	cpx_hooks:set_hook(mn_get_skills, get_skills, ?MODULE, get_skills, [], 100),

	cpx_hooks:set_hook(mn_get_client, get_client, ?MODULE, get_client, [], 100),
	cpx_hooks:set_hook(mn_get_clients, get_clients, ?MODULE, get_clients, [], 100),
	ok.
...
-spec(get_queue/1 :: (Queue :: string()) -> {ok, #call_queue{}} | {'noexists', any()} | 'noexists').
get_queue(Name) ->
	try integration:get_queue(Name) of
		none ->
			?DEBUG("integration has no such queue ~p", [Name]),
			destroy_queue(Name),
			noexists;
		{ok, Name, Weight, Skills, Recipe, HoldMusic, Group} ->
			?DEBUG("integration found queue ~p", [Name]),
...
-spec(local_get_queue/1 :: (Queue :: string()) -> {ok, #call_queue{}} | {'noexists', any()} | 'noexists').
local_get_queue(Queue) ->
	F = fun() -> 
...
get_merged_queue(Queue) ->
	case get_queue(Queue) of
		{ok, QueueRec} when is_record(QueueRec, call_queue) ->
			case get_queue_group(QueueRec#call_queue.group) of
				{ok, GroupRec} when is_record(GroupRec, queue_group) ->
...
-spec(get_queues/0 :: () -> {ok, [#call_queue{}]}).
get_queues() -> 
...
-spec(get_queue_group/1 :: (Name :: string()) -> {ok, #queue_group{}}).
get_queue_group(Name) ->
	try integration:get_queue_group(Name) of
		none ->
			?DEBUG("integration has no such queue group ~p", [Name]),
			destroy_queue_group(Name),
			{atomic, []};
		{ok, Name, Recipe, Skills, Sort, Protected} ->
			?DEBUG("integration found queue group ~p", [Name]),
			...
			new_queue_group(QueueGroup),
			local_get_queue_group(Name);
		{error, nointegration} ->
			?DEBUG("No integration, falling back for ~p", [Name]),
			local_get_queue_group(Name)
	catch
...
%% @doc skip integration, get a `#queue_group{}' named `Name' directly
-spec(local_get_queue_group/1 :: (Name :: string()) -> {ok, #queue_group{}}).
local_get_queue_group(Name) ->
	F = fun() ->
		QH = qlc:q([begin
	...
	case mnesia:transaction(F) of
		{atomic, [QueueGroup]} -> {ok, QueueGroup};
		_ -> error
	end.

%% @doc Gets all `#queue_group{}' in a list sorted by group.
-spec(get_queue_groups/0 :: () -> {ok, [#queue_group{}]}).
get_queue_groups() ->
	F = fun() ->
		QH = qlc:q([begin
			Oldrecipe =X#queue_group.recipe,
...
-spec(get_skill/1 :: (Skill :: atom() | string()) -> {ok, #skill_rec{}} | 'undefined').
...
get_skill(Skill) when is_atom(Skill) ->
	try integration:get_skill(Skill) of
		none ->
			?DEBUG("integration has no such skill ~p", [Skill]),
			F = fun() -> mnesia:delete({skill_rec, Skill}) end,
			{atomic, ok} = mnesia:transaction(F),
			undefined;
		{ok, Skillatom, Skillname, Skillprotected, Skilldesc, Group} ->
			Rec = #skill_rec{atom = Skillatom, name = Skillname, protected = Skillprotected, description = Skilldesc, group = Group},
			new_skill(Rec),
			local_get_skill(Skill);
		{error, nointegration} ->
			?DEBUG("No integration, falling back for ~p", [Skill]),
			local_get_skill(Skill)
	catch
		throw:{badreturn, Err} ->
...
-spec(local_get_skill/1 :: (Skill :: atom()) -> {ok, #skill_rec{}} | 'undefined').
local_get_skill(Skill) when is_atom(Skill) ->
	F = fun() ->
...
-spec(get_skills/0 :: () -> {ok, [#skill_rec{}]}).
get_skills() ->
	F = fun() ->
...
%% if possible.
-spec(get_client/1 :: (Label :: string() | 'undefined') -> {ok, #client{}} | 'none').
get_client(Label) ->
...
merge_client_options(Given) ->
	{ok, #client{options = Defaults}} = get_default_client(),
			...
			fun(Test) ->
				{
					...
					fun() ->
						...
						{ok, Gottennew} = get_skills("Talky"),
						{ok, Gottenold} = get_skills("Language"),
...
skill_rec_integration_test_() ->
	{
		...
		[
			fun(Mock) ->
				{"Get existing skill from integration",
				fun() ->
					gen_server_mock:expect_call(Mock, fun({get_skill, skill}, _, State) -> {ok, {ok, skill, "skill", false, "desc", "misc"}, State} end),
					?assertMatch(
						{ok, #skill_rec{
							atom = skill,
							name = "skill",
			...
			fun(Mock) ->
				{"Get non-existing skill from integration",
				fun() ->
...
client_rec_test_() ->
	...
	fun() ->
		...
		{ok, #client{options = Dasopts}} = get_client("test1"),
	...
	{"integration mocking",
		...
		[fun(Mock) ->
			...
			fun() ->
				...
				?assertMatch({ok, #client{label = "client", id = "0020004", options = []}}, get_client(label, "client")),
				?assertMatch({ok, #client{label = "client", id = "0020004", options = []}}, local_get_client(label, "client"))
			end}
		end,
		fun(Mock) ->
			...
			fun() ->
				...
				?assertMatch({ok, #client{label = "client", id = "0020004"}}, local_get_client(label, "client")),
		...
		fun(Mock) ->
			...
			fun() ->
				...
				new_client(#client{label = "client", id = "0020004"}),
				?assertMatch({ok, #client{label = "client", id = "0020004"}}, get_client("client"))
			end}
		end]}
	}]}.
