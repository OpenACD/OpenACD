...
task :roll_release => [:submodules] do
	...
	sh "gzip -f OpenACD-#{version}.tar"
end

namespace :test do
	desc "Compile .beam files with -D TEST and +debug_info => debug_ebin"
	...
	task :build_plt do
		...
		sh "dialyzer --build_plt -r #{foo}/erts*/ebin #{foo}/kernel*/ebin #{foo}/stdlib*/ebin #{foo}/mnesia*/ebin #{foo}/crypto*/ebin #{foo}/eunit*/ebin #{foo}/compiler*/ebin #{foo}/ssl*/ebin #{foo}/public_key*/ebin"
	end
end
