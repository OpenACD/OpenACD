...
process_agent(Agent, Command) ->
	...
	if Command =:= "ADD" ->
		agent_auth:add_agent(erlang:binary_to_list(Name), erlang:binary_to_list(Firstname), erlang:binary_to_list(Lastname), erlang:binary_to_list(Pin), erlang:binary_to_list(Skills), agent, erlang:binary_to_list(Group));
	Command =:= "DELETE" ->
