...
function resizespan(spn) {
	var fontmax=140;
	var curfont = parseInt(spn.style.fontSize);
	var parentht = parseInt(spn.parentNode.clientHeight) * 0.95;
	var parentwd = parseInt(spn.parentNode.clientWidth) * 0.95;
	...
	if (isNaN(curfont) || (curfont < 1)) curfont = 10;
	var step = parseInt( curfont / 2 + 0.5);
	...
	while (!done) {
		...
		hdir = sign(hdiff);
		wdir = sign(wdiff);
		if (hdir == -1) diff = hdiff;
		if (wdir == -1) diff = wdiff;
		if ((wdir > 0) && (hdir > 0)) closest = curfont;
		if (hdir == wdir) diff = (Math.abs(hdiff) < Math.abs(wdiff)) ? hdiff : wdiff;	
		prevstep = laststep;
		laststep = step;
		if (sign(diff) == sign(lastdiff)) {
			step = step * 2;
		} else {
			step = parseInt(step / 2 + 0.5);
			//step = parseInt(diff / 2 + 0.5);
		}
		if ((step == laststep) || (step == prevstep)) step = parseInt(step / 2 + 0.5);
...
function resizeall() {
	...
	for (var i=0;i<spans.length;i++) {
		if (spans[i].attributes['resize']) resizespan(spans[i]);
	}
...
function swapstat() {
	...
	for (var i = 0; i < divs.length; i++) {
		if (divs[i].className == 'stat') {
			stats.push(divs[i]);
			if (active != -1) divs[i].style.zIndex = -1;
			if (divs[i].style.zIndex > 0) active = stats.length - 1;
		}
	}
	if (active == -1) {
		...
		if (nextstat >= stats.length) nextstat = 0;
...
function swapalert(alertdiv) {
	...
	if (divs.length == 0) {
		if (specialtext.length > 0) {
			...
			nextspecialtext += 1;
			if (nextspecialtext >= specialtext.length) nextspecialtext = 0;
	...
	for (i = 0; i < divs.length; i++) {
		...
		if (parseInt(divs[i].style.zIndex) > 0 )
	...
	if (active == -1) {
		...
		if (nextalert >= alerts.length) nextalert = 0;
