	...
	queueDashboard.Queue.prototype.consumeEvent = function(data){
		...
		if(this.medias[data.id]){
			...
			if(data.action == 'drop'){
				...
				this.medias[data.id] = new dashboard.Media(data);
				this.calls++;
			} else {
				this._limboMedias[data.id] = new dashboard.Media(data);
			}
		}
...
queueDashboard.mediaSub = dojo.subscribe('dashboard/supevent/media', queueDashboard, function(action, supevent){
	if(! this.filterSupevent(supevent)){
		return false;
	}
	var supcp = supevent;
