...
handle_answer(Apid, Callrec, #state{statename = inqueue_ringing} = State) ->
	...
	Agent = agent:dump_state(Apid),
	case freeswitch:api(State#state.cnode, uuid_bridge, UUID ++ " " ++ RingUUID) of
		{ok, _} ->
			RecPath = case cpx_supervisor:get_archive_path(Callrec) of
				...
				Path ->
					...
					freeswitch:api(State#state.cnode, uuid_setvar, UUID ++ " RECORD_APPEND true"),
					freeswitch:api(State#state.cnode, uuid_record, UUID ++ " start " ++ Path ++ ".wav"),
					freeswitch:bgapi(State#state.cnode, uuid_setvar, RingUUID ++ " RECORD_APPEND true"),
					freeswitch:bgapi(State#state.cnode, uuid_record, RingUUID ++ " start " ++ Path ++ "." ++ Agent#agent.login ++ "." ++ Agent#agent.login ++ ".wav"),
					Path
			end,
			agent:conn_cast(Apid, {mediaload, Callrec, [{<<"width">>, <<"800px">>}, {<<"height">>, <<"600px">>}, {<<"title">>, <<"Server Boosts">>}]}),
			{ok, State#state{statename = oncall, agent_pid = Apid, ringuuid = RingUUID, record_path = RecPath, queued = false}};
		{error, "-ERR Invalid uuid\n"} ->
			proc_lib:spawn(fun() ->
				case freeswitch:api(State#state.cnode, uuid_exists, UUID) of
					{ok, "true"} ->
						ok;
					NotTrue ->
...
handle_queue_transfer(Call, #state{cnode = Fnode} = State) ->
	case State#state.record_path of
		...
		Path ->
			...
			freeswitch:api(Fnode, uuid_record, State#state.uuid ++ " stop " ++ Path ++ ".wav")
	end,
...
handle_cast(toggle_hold, Call, #state{statename = Statename} = State)
	...
	#state{cnode = Fnode, moh = Muzak, ringuuid = Ringid, uuid = UUID} = State,
	#call{id = Callid} = Call,
	...
	freeswitch:api(Fnode, uuid_setvar_multi, UUID ++ " hangup_after_bridge=false;park_after_bridge=true"),
	...
	case Muzak of
		none -> ok;
		_ ->
			fs_send_execute(Fnode, UUID, "playback", "local_stream://" ++ Muzak)
	end,
	...
	case State#state.record_path of
		...
		Path ->
			...
			freeswitch:api(Fnode, uuid_record, UUID ++ " stop " ++ State#state.record_path ++ ".wav")
	end,
...
handle_cast(toggle_hold, Call, #state{statename = oncall_hold} = State) ->
	...
	#call{id = Callid} = Call,
	UUID = State#state.uuid,
	freeswitch:api(Fnode, uuid_bridge, UUID ++ " " ++ Ringid),
	freeswitch:api(Fnode, uuid_setvar_multi, UUID ++ " hangup_after_bridge=true;park_after_bridge=false"),
	case State#state.record_path of
		undefined -> ok;
		Path ->
